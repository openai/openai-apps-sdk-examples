...
This server pairs with the `examples/kitchen-sink-lite/ui/kitchen-sink-lite` widget bundle. It exposes two
tools:
...
ASSET_BASE_URL_PLACEHOLDER = "__ASSET_BASE_URL__"
...
def load_widget_html() -> str:
    ...
    if direct.exists():
        return direct.read_text(encoding="utf8").replace(
            ASSET_BASE_URL_PLACEHOLDER, asset_base_url()
        )

    candidates = sorted(ASSETS_DIR.glob("kitchen-sink-lite-*.html"))
    if candidates:
        return candidates[-1].read_text(encoding="utf8").replace(
            ASSET_BASE_URL_PLACEHOLDER, asset_base_url()
        )
...
def asset_base_url() -> str:
    explicit_asset_base_url = (os.getenv("ASSET_BASE_URL") or "").strip()
    if explicit_asset_base_url:
        return explicit_asset_base_url.rstrip("/")

    explicit_public_base_url = (os.getenv("PUBLIC_BASE_URL") or "").strip()
    if explicit_public_base_url:
        return f"{explicit_public_base_url.rstrip('/')}/assets"

    port = int(os.getenv("PORT", "8000"))
    return f"http://localhost:{port}/assets"
