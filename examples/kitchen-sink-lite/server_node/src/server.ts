...
function readWidgetHtml(assetBaseUrl: string): string {
...
function createKitchenSinkServer(assetBaseUrl: string): Server {
      ...
      contents: [
        {
          uri: TEMPLATE_URI,
          mimeType: MIME_TYPE,
          text: readWidgetHtml(assetBaseUrl),
          _meta: toolDescriptorMeta(),
        },
      ],
    })
...
async function handleSseRequest(req: IncomingMessage, res: ServerResponse) {
  res.setHeader("Access-Control-Allow-Origin", "*");
  const server = createKitchenSinkServer(getAssetBaseUrl(req, port));
