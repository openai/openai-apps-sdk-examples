...
Build the widget assets from the repo root:
...
Then start the shopping-cart MCP server (from this directory):

```bash
python main.py
```

The server exposes `GET /mcp` for SSE and `POST /mcp/messages?sessionId=...` for follow-up messages, and serves widget bundles at `/assets/...`.
