...
def _asset_base_url() -> str:
    ...
    if explicit_asset_base_url:
        return _normalize_base_url(explicit_asset_base_url)

    explicit_public_base_url = (os.getenv("PUBLIC_BASE_URL") or "").strip()
    if explicit_public_base_url:
        return f"{_normalize_base_url(explicit_public_base_url)}/assets"

    resource_server_url = (os.getenv("RESOURCE_SERVER_URL") or "").strip()
...
def _load_widget_html(component_name: str) -> str:
    ...
    if html_path.exists():
        return html_path.read_text(encoding="utf8").replace(
            ASSET_BASE_URL_PLACEHOLDER, _asset_base_url()
        )

    fallback_candidates = sorted(ASSETS_DIR.glob(f"{component_name}-*.html"))
    if fallback_candidates:
        return fallback_candidates[-1].read_text(encoding="utf8").replace(
            ASSET_BASE_URL_PLACEHOLDER, _asset_base_url()
        )
